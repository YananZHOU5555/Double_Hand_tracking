# EgoInfinity-style Hand Pipeline

Current implementation state is tracked in:

```text
local_pipelines/egoinfinity_hand_pipeline/PIPELINE_IMPLEMENTATION.md
```

This is an experimental, isolated hand-tracking pipeline. It is intentionally
separate from `local_pipelines/double_hand_pipeline` so the current gripper/IK
pipeline remains usable while we migrate the hand stack.
...
7. Export back to LFV CSV/NPZ/HTML for gripper mapping.
...
INSTALL_MEMFOF=true PREFETCH_MEMFOF=true \
  bash local_pipelines/egoinfinity_hand_pipeline/setup_egoinfinity_strict_components.sh
```
...
/home/yannan/miniforge3/envs/wilor_lfv/bin/python \
  local_pipelines/egoinfinity_hand_pipeline/check_egoinfinity_strict_components.py \
...
```text
local_pipelines/egoinfinity_hand_pipeline/strict_components_status.json
```
