...
def main() -> int:
    ...
    phase_b = Path(args.phase_b_npz).expanduser().resolve() if args.phase_b_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_b_track_postprocess" / "wilor_handresults_phase_b.npz"
    )
    depth_summary = Path(args.foundation_depth_summary).expanduser().resolve() if args.foundation_depth_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "foundationstereo_depth" / "foundationstereo_depth_summary.json"
    )
    phase_c0b_summary = Path(args.phase_c0b_summary).expanduser().resolve() if args.phase_c0b_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "foundationstereo_depth_stabilized" / "foundationstereo_depth_stabilized_summary.json"
    )
    phase_c0b_frame_csv = Path(args.phase_c0b_frame_csv).expanduser().resolve() if args.phase_c0b_frame_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "foundationstereo_depth_stabilized" / "foundationstereo_depth_stabilized_frames.csv"
    )
    phase_c0b_correction_csv = Path(args.phase_c0b_correction_csv).expanduser().resolve() if args.phase_c0b_correction_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "foundationstereo_depth_stabilized" / "depth_stabilize_corrections.csv"
    )
    phase_c_npz = Path(args.phase_c_npz).expanduser().resolve() if args.phase_c_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_align" / "wilor_handresults_phase_c_depth_aligned.npz"
    )
    phase_c_summary = Path(args.phase_c_summary).expanduser().resolve() if args.phase_c_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_align" / "phase_c_alignment_summary.json"
    )
    phase_c_quality_csv = Path(args.phase_c_quality_csv).expanduser().resolve() if args.phase_c_quality_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_align" / "phase_c_alignment_quality.csv"
    )
    phase_c1b_npz = Path(args.phase_c1b_npz).expanduser().resolve() if args.phase_c1b_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_smooth" / "wilor_handresults_phase_c1b_depth_smooth.npz"
    )
    phase_c1b_summary = Path(args.phase_c1b_summary).expanduser().resolve() if args.phase_c1b_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_smooth" / "depth_smooth_summary.json"
    )
    phase_c1b_quality_csv = Path(args.phase_c1b_quality_csv).expanduser().resolve() if args.phase_c1b_quality_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_smooth" / "depth_smooth_quality.csv"
    )
    phase_c1b_track_csv = Path(args.phase_c1b_track_csv).expanduser().resolve() if args.phase_c1b_track_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_depth_smooth" / "depth_smooth_track_summary.csv"
    )
    phase_c1c_npz = Path(args.phase_c1c_npz).expanduser().resolve() if args.phase_c1c_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_motion_infiller" / "wilor_handresults_phase_c1c_motion_infilled.npz"
    )
    phase_c1c_summary = Path(args.phase_c1c_summary).expanduser().resolve() if args.phase_c1c_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_motion_infiller" / "motion_infiller_summary.json"
    )
    phase_c1c_quality_csv = Path(args.phase_c1c_quality_csv).expanduser().resolve() if args.phase_c1c_quality_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_motion_infiller" / "motion_infiller_quality.csv"
    )
    phase_c2_npz = Path(args.phase_c2_npz).expanduser().resolve() if args.phase_c2_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mano_smooth" / "wilor_handresults_phase_c2_mano_smooth.npz"
    )
    phase_c2_summary = Path(args.phase_c2_summary).expanduser().resolve() if args.phase_c2_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mano_smooth" / "mano_smoothing_summary.json"
    )
    phase_c2_quality_csv = Path(args.phase_c2_quality_csv).expanduser().resolve() if args.phase_c2_quality_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mano_smooth" / "mano_smoothing_quality.csv"
    )
    phase_c3_npz = Path(args.phase_c3_npz).expanduser().resolve() if args.phase_c3_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mesh_visibility" / "wilor_handresults_phase_c3_mesh_visibility.npz"
    )
    phase_c3_summary = Path(args.phase_c3_summary).expanduser().resolve() if args.phase_c3_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mesh_visibility" / "mano_mesh_visibility_summary.json"
    )
    phase_c3_joint_csv = Path(args.phase_c3_joint_csv).expanduser().resolve() if args.phase_c3_joint_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mesh_visibility" / "mano_mesh_visibility_joints.csv"
    )
    phase_c3_candidate_csv = Path(args.phase_c3_candidate_csv).expanduser().resolve() if args.phase_c3_candidate_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_mesh_visibility" / "mano_mesh_visibility_candidates.csv"
    )
    phase_c4_npz = Path(args.phase_c4_npz).expanduser().resolve() if args.phase_c4_npz else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_visibility_depth_realign" / "wilor_handresults_phase_c4_visibility_depth_realign.npz"
    )
    phase_c4_summary = Path(args.phase_c4_summary).expanduser().resolve() if args.phase_c4_summary else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_visibility_depth_realign" / "visibility_depth_realign_summary.json"
    )
    phase_c4_quality_csv = Path(args.phase_c4_quality_csv).expanduser().resolve() if args.phase_c4_quality_csv else (
        session / "quality" / "egoinfinity_hand_alignment_pipeline" / "stages" / "phase_c_visibility_depth_realign" / "visibility_depth_realign_quality.csv"
    )
